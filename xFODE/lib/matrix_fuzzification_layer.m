...
%
    ...
    elseif(membership_type == "gauss2mf") 
        ...
        output = custom_gauss2mf(input_matrix, left_sigmas, right_sigmas, centers); % calculating fuzzified values
    elseif(membership_type == "c-gauss2mf")
        input_matrix = repmat(x, number_of_rule, 1, 1); % expanding the input matrix by the number of rules
        [centers, left_sigmas, right_sigmas] = calculate_centers(learnable_parameters.leftmost_center,learnable_parameters.input_sigmas);
        output = custom_c_gauss2mf(input_matrix, left_sigmas, right_sigmas, centers); % calculating fuzzified values
    elseif membership_type == "trimf"
        output = matrix_fuzzification_layer_triangular(x, learnable_parameters, number_of_rule); % calculating fuzzified values
    end
